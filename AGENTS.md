<!-- From: /ephstorage/vln_code/InternNav/AGENTS.md -->
...
| License | `LICENSE` and `README.md` state MIT for code; `setup.py` states `Apache 2.0`. Note the discrepancy. |
...
- `gym==0.26.2`, `gymnasium==0.29.1` — environment interfaces.
- `opencv-python-headless`, `open3d`, `numpy>=1.26,<2.0` — perception / geometry.
- `pytest`, `pytest-timeout` — testing.

Dependencies are split across five requirement files:
...
```text
.
├── internnav/                    # Main Python package (~377 Python files incl. submodule)
...
│   ├── train/base_train/         # Unified training entry point and configs
...
Some files still append the legacy path `./src/diffusion-policy` to `sys.path`:

- `scripts/train/base_train/train.py`
- `internnav/habitat_extensions/vln/habitat_default_evaluator.py`
- `internnav/habitat_extensions/vln/habitat_vln_evaluator.py`

Other files already use the correct current path:

- `scripts/eval/eval.py`
- `scripts/eval/start_server.py`

The submodule actually lives at `third_party/diffusion-policy`. If you hit import errors, verify that the path matches your checkout.
...
- Observations are `pickle` + `base64` encoded over JSON.
...
- Unified training entry point: `scripts/train/base_train/train.py`.
...
- It loads the matching `ExpCfg` from `scripts/train/base_train/configs/` and calls `trainer.train()` from the `transformers.Trainer` family.
- InternVLA-N1 training uses `scripts/train/qwenvl_train/` with DeepSpeed configs (`zero2.json`, `zero3.json`, `zero3_offload.json`).
...
- `internnav/configs/evaluator/__init__.py` — `EnvCfg`, `TaskCfg`, `EvalCfg`, `EvalDatasetCfg`, `RobotCfg`, `SceneCfg`, `MetricCfg`.
...
- `tests/function_test/e2e_test.py` — wraps server and evaluator tests via subprocess; writes `test_result.json` and `../total_result.jsonl`.
...
The IROS 2025 on-site SDK is located at `scripts/iros_challenge/onsite_competition/sdk/` and contains camera, control, and real-world environment wrappers.
...
- The FastAPI `AgentServer` (`scripts/eval/start_server.py`) pickles and unpickles observations over HTTP. Do not expose it to untrusted networks; unpickling untrusted payloads is unsafe.
...
- The IROS on-site SDK stream server (`scripts/iros_challenge/onsite_competition/sdk/stream.py`) also binds to `0.0.0.0` without authentication.
...
If you change build steps, test commands, style tools, submodules, or deployment paths, update this `AGENTS.md` to keep it accurate.
